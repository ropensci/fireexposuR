...
```{r example1c, fig.width=9, fig.height=8,  fig.show="hold", out.width="50%"}
# visualize in a map
...
```{r example2, fig.width=9, fig.height=8}
...
```{r example3d, echo = FALSE, fig.show="hold", out.width="50%", fig.height=8}
...
```{r example3f, fig.show="hold", out.width="50%", fig.height=8}
# read example area of interest
polygon_path <- system.file("extdata", "polygon.shp", package ="fireexposuR")
aoi <- terra::vect(polygon_path)
...
```{r example3g, fig.show="hold", out.width="50%", fig.height=8}
...
Here we can see that there are 4 values that are extremely exposed (>45%) to both short- and long- range embers. 
