...
Second, consider which analyses you are interested in using the package for. Determining the scope will inform the required spatial resolution and extent of the input data, which is further documented in the vignette `vignette("prep-input-data")`.
...
This example code demonstrates how to use the package to compute wildfire exposure at two different scales of wildfire transmission and then export the results as a .tif file. For advice on preparing the input data see the `vignette("prep-input-data")`.
...
point_value <- terra::vect("point_value.shp")

# read point feature vector of value locations
multiple_values <- terra::vect("multiple_values.shp")
...
terra::writeVector(transects_point, "transects_point.shp")
...
for (i in seq_len(nrow(multiple_values))) {
  ...
  terra::writeVector(dir_exp_point, file_name)
}
```

### Example: Conduct an exposure assessment for a local area

Assessing wildfire exposure for a local area can be conducted for multiple scales of wildfire transmission. Some examples of where a localized assessment would be useful are: a community, a neighbourhood, or anywhere values are concentrated. For advice on preparing the input data see the `vignette("prep-input-data")`.
...
This example code demonstrates how to use the package to compute wildfire exposure at two different scales of wildfire transmission and then visualize the results in R for a localized area of interest. For advice on preparing the input data see the `vignette("prep-input-data")`.
...
area_of_interest <- terra::vect("area_of_interest.shp")
...
hazard_a <- terra::rast("hazard_a.tif")

# read non-burnable landscape raster: option A
no_burn_a <- terra::rast("no_burn_a.tif")

# read long-range ember hazard raster: option B
hazard_b <- terra::rast("hazard_b.tif")

# read non-burnable landscape raster: option B
no_burn_b <- terra::rast("no_burn_b.tif")
...
The vignette `vignette("prep-input-data")` provides guidance, suggestions, and examples of how to best source and prepare these datasets.
